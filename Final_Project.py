...
mcp = Adafruit_MCP3008.MCP3008(spi=SPI.SpiDev(SPI_PORT, SPI_DEVICE)) # Setup for the Adafruit ACD chip
...
def detect_taps(signal):
    ...
    for i in range(1, len(signal)):
        if signal[i] > 500 and signal[i - 1] <= 500:
            # Start of a new sound wave
            if tap_start_time is None:
                tap_start_time = time.time()
        elif signal[i] <= 0 and signal[i - 1] > 500:
            # End of a sound wave
